...
void startServer(GameSettings settings) {
    ...
    serverCommands(settings);
}

/// Server administration commands helper
void serverCommands(ref GameSettings settings) {
    foreach(line; stdin.byLine) {
        logMessage(to!string(line), fg.white);
        string[] splitted_line = to!string(line).split(" ");
        if (splitted_line.length != 0) {
            processServerCommand(splitted_line, settings);
        }
    }
}

/// Process the server command
void processServerCommand(string[] args, ref GameSettings settings) {
    logMessage("Command: " ~ args[0], fg.white);
    switch(args[0]) {
        case "server_info":
        ...
        case "player_info":
            if (args.length == 2) {
                if (canFind(["Player_0", "Player_1", "Player_2", "Player_3"], args[1])) {
                    getPlayerInfo(args[1], settings);
                }
            }
            break;
        ...
        case "move_pawn":
            ...
            data_to_send.object["source_field"] = JSONValue(args[3]);
            data_to_send.object["destination_field"] = JSONValue(args[4]);
            /// Update pawn location on server side
            settings.players[args[1]].pawns[args[2]] = args[4];
            data_to_send.object["to_delete_pawns"] = JSONValue("");
            data_to_send.object["pawn_to_move"] = JSONValue(args[2]);
            foreach(player_to_send; settings.players) {
                player_to_send.socket.send(data_to_send.toString());
            }
            break;
        ...
        case "set_dice_result":
            settings.rollResult = to!int(args[1]);
            JSONValue data_to_send = ["action": "roll_result"];
            data_to_send.object["result"] = JSONValue(to!int(args[1]));
            foreach(player_to_send; settings.players) {
                player_to_send.socket.send(data_to_send.toString());
            }
            break;
        ...
        default:
            logMessage("Unrecognized command.", fg.white);
            break;
    }
}
...
void writeHelpMessage() {
    logMessage("Possible commands:", fg.white);
    logMessage("server_info - informations about server settings.", fg.white);
    logMessage("player_info {player} - information about player.", fg.white);
    logMessage("move_pawn {player} {pawn} {from} {to} - move pawn from one field to another.", fg.white);
    logMessage("set_dice_result {number(1:6)} - sets a dice result.", fg.white);
    logMessage("set_current_player {player} - sets a current player.", fg.white);
    logMessage("help - show this help information.", fg.white);
}

/// Get player info
void getPlayerInfo(string player, ref GameSettings settings) {
    logMessage("Nickname: " ~ to!string(settings.players[player].nickname), fg.white);
    logMessage("Next player: " ~ to!string(settings.players[player].nextPlayer), fg.white);
    logMessage("Ready: " ~ to!string(settings.players[player].ready), fg.white);
    logMessage("Socket: " ~ to!string(settings.players[player].socket), fg.white);
    logMessage("Color: " ~ to!string(settings.players[player].color), fg.white);
    logMessage("Remaining rolls: " ~ to!string(settings.players[player].remainingRolls), fg.white);
    logMessage("Three chances: " ~ to!string(settings.players[player].threeChances), fg.white);
    logMessage("Player can end turn: " ~ to!string(settings.players[player].playerCanEndTurn), fg.white);
    logMessage("Pawns: " ~ to!string(settings.players[player].pawns), fg.white);
}

/// Get the server informations
void getServerInfo(ref GameSettings settings) {
    logMessage("Game started: " ~ to!string(settings.gameStarted), fg.white);
    logMessage("Ready players: " ~ to!string(settings.readyPlayers), fg.white);
    logMessage("Current player: " ~ to!string(settings.currentPlayer), fg.white);
    logMessage("Roll result: " ~ to!string(settings.rollResult), fg.white);
    logMessage("Players: " ~ to!string(settings.players), fg.white);
    logMessage("Fields: " ~ to!string(settings.fields), fg.white);
}
                ...
                if (clickedEndField(field_clicked)) {
                    ...
                    if (clickedCorrectBaseField(field_clicked, settings.players[player].color)) {
                        ...
                        if (clickedEndField(destinationField)) {
                            ...
                            normalFieldClicked(player, destinationField, pawn_clicked, settings);
                        }
                    }
                }
            }
...
/// If Player won a game send a message about that and reset everything to the start state
void ifPlayerWonGame(string player, ref GameSettings settings) {
    if (playerWin(player, settings)) {
        logMessage("Player " ~ player ~ " won a game!", fg.green);
...
void normalFieldClicked(string player, string destinationField, string pawn_clicked, ref GameSettings settings) {
    ...
    if (!(destinationField is null)) {
        ...
        data_to_send.object["pawn_to_move"] = JSONValue(pawn_clicked);
        data_to_send.object["to_delete_pawns"] = JSONValue("");
        FieldPawnsOwner fieldWithEnemyPawns = getEnemyPlayerOnStartPoint(player, destinationField, field_pawns);
        logMessage("Enemy on field: " ~ to!string(fieldWithEnemyPawns), fg.red);
        /// There is some enemy pawns already on field and this is not their start field
        if (!(fieldWithEnemyPawns.owner is null)) {
            data_to_send.object["to_delete_pawns"] = JSONValue(fieldWithEnemyPawns.pawns);
            /// Update `field owner pawns` to their base fields
            foreach (pawn, field; settings.players[fieldWithEnemyPawns.owner].pawns) {
                foreach (field_pawn; fieldWithEnemyPawns.pawns) {
                    if (pawn == field_pawn) {
                        settings.players[fieldWithEnemyPawns.owner].pawns[field_pawn] = (getPawnBaseField(field_pawn));
                    }
                }
            }
            /// Check if `field owner` has other pawns on game board otherwise he should have again 3 chances
            bool should_be_3_rolls = true;
            foreach (pawn, field; settings.players[fieldWithEnemyPawns.owner].pawns) {
                logMessage(
                    to!string(pawn) ~ ": " ~ to!string(field) ~ " " ~ to!string(checkPawnOnPlayBoard(field)),
                    fg.red
                );
                if (checkPawnOnPlayBoard(field)) {
                    should_be_3_rolls = false;
                    break;
                }
            }
            logMessage("Should be 3 rolls: " ~ to!string(should_be_3_rolls), fg.red);
            settings.players[fieldWithEnemyPawns.owner].threeChances = should_be_3_rolls;
        }
        if (playerWin(player, settings)) {
            logMessage("Player " ~ player ~ " won a game!", fg.green);
            JSONValue player_won_data = ["action": "player_win"];
            player_won_data.object["player"] = JSONValue(player);
            foreach(player_to_send; settings.players) {
                player_to_send.socket.send(player_won_data.toString());
            }
            resetGame(settings);
        } else {
            foreach(player_to_send; settings.players) {
            ...
            /// If Player has no more rolls he can end the turn
            if (settings.players[player].remainingRolls == 0) {
                settings.players[player].playerCanEndTurn = true;
            }
        }
        ...
        JSONValue data_to_send = ["action": "error"];
        data_to_send.object["message"] = JSONValue("The field you are trying to move to, doesn't exist!");
        settings.players[player].socket.send(data_to_send.toString());
            ...
            if (pawn_clicked == pawn) {
                ...
                if (checkPawnOnPlayBoard(field)) {
                    if (!pawnCanMove(field, player, settings.rollResult, settings)) {
                        /// Player can end turn when he has a pawn on game board but he can't move it the rolled
                        /// amount of fields
                        settings.rollResult = 0;
                        settings.players[player].playerCanEndTurn = true;
                    }
...
/// or `FieldPawnsOwner` with owner set to null
...
void destinationFieldOnFinishLine(
    ...
    if (field_owner is null) {
        ...
        /// Player can end turn when he has a pawn on game board but he can't move it the rolled amount of fields
        if (!player_can_move) {
            settings.rollResult = 0;
            settings.players[player].playerCanEndTurn = true;
        }
            ...
            if (settings.players[player].playerCanEndTurn) {
                ...
                if (settings.rollResult == 0) {
                    ...
                    if (clickedEnemyBaseField(field_clicked, settings.players[player].color)) {
                        JSONValue data_to_send = ["action": "error"];
                        data_to_send.object["message"] = JSONValue("It's not a field in your color!");
                        settings.players[player].socket.send(data_to_send.toString());
                        return false;
                    }
                }
...
/// Game is over reset it to initial state
void resetGame(ref GameSettings settings) {
    settings.gameStarted = false;
    settings.players.clear();
    settings.readyPlayers = 0;
...
bool playerWin(string player, ref GameSettings settings) {
    ...
    logMessage("Pawns on finish line counter: " ~ to!string(counter), fg.light_cyan);
    if (counter == 4) {
        return true;
    } else {
        return false;
...
string getNextField(string field_clicked, string player, ref GameSettings settings) {
    ...
    if (field_clicked == "gray_20" && player == "Player_2") {
        return settings.fields[field_clicked].blueNextField;
    }
    if (field_clicked == "gray_11" && player == "Player_3") {
...
bool clickedEnemyBaseField(string field, string player_color) {
    ...
    if (player_color == "blue") {
        if(canFind([
            ...
            "yellow_base_3", "yellow_base_4", "red_base_1", "red_base_2", "red_base_3", "red_base_4"
        ], field)) {
            return true;
        }
    }
