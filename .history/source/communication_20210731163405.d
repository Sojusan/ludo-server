                            ...
                            if (clickedEnemyBaseField(field_clicked, settings.players[player].color)) {
                                ...
                                if (clickedEndField(field_clicked)) {
                                    ...
                                    if (clickedCorrectBaseField(field_clicked, settings.players[player].color)) {
                                        string destination_field = getStartField(player);
                                        if (destination_field is null) {
                                            JSONValue data_to_send = ["action": "error"];
                                            data_to_send.object["message"] = JSONValue(
                                            ...
                                            if (settings.rollResult == 6) {
                                                ...
                                                data_to_send.object["destination_field"] = JSONValue(destination_field);
                                        ...
                                        if (settings.rollResult != 0) {
                                            ...
                                            if (clickedEndField(destinationField)) {
                                                ...
                                                string[] field_owners = [];
                                                foreach (nickname, player; settings.players[player].pawns) {
                                                    if (field == destinationField) {
                                                        field_owners ~= pawn;
                                                        break;
                                                    }
                                                ...
                                                if (field_owners.length != 0) {
                                                    /// Check if this owners are current players pawns
                                                    bool isPlayerOwner = false;
                                                    foreach (owner; field_owners) {
                                                        foreach (pawn, field; settings.players[player].pawns) {
                                                            if (owner == pawn) {
                                                                isPlayerOwner = true;
                                                                break;
                                                ...
                                                /// Jeśli na pole z innym pionkiem to trzeba sprawdzić czy zbity gracz ma jakieś
                                                /// pionki na polu gry, jak nie to dać mu zmienną że ma trzy próby
                                            }
                                        } else {
                                            JSONValue data_to_send = ["action": "error"];
                                            data_to_send.object["message"] = JSONValue(
