...
FieldPawnsOwner getEnemyPlayerOnStartPoint(
    string player, string destination_field, FieldPawnsOwner[] field_info, ref GameSettings settings
) {
    foreach (field; field_info) {
        /// Check if this is the player start field (we can't destroy the pawns on start field)
        if (isPlayerStartField(field.owner, destination_field)) {
            continue;
        } else {
            /// If on field are actually pawns of the current player 
