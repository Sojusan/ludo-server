...
/// Class for getting information about pawns on the field with a pl
class FieldPawnsOwner {
...
void normalFieldClicked(string player, string destinationField, string pawn_clicked, ref GameSettings settings) {
    ...
    if (!(destinationField is null)) {
        string[string] field_pawns;
        string player_field_owner = null;
        /// Check if destination field is already taken by other player
        foreach (nickname, _player; settings.players) {
            ...
            /// This is not possible to be multiple players pawns on one field
        ...
        /// There is some pawns already on field
        if (!(player_field_owner is null)) {
            /// Check if field owner is a current player (then pawns are not removed)
            if (player_field_owner == player) {
                data_to_send.object["to_delete_pawns"] = JSONValue("");
            } else {
                /// Check if
                if (isPlayerStartField(player_field_owner, destinationField)) {

                }
                data_to_send.object["to_delete_pawns"] = JSONValue(field_pawns);
