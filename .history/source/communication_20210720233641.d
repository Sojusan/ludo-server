                ...
                if(readSet.isSet(listener)) {
                    ...
                    if (connectedClients.length > 3) {
                        ...
                        if (!settings.gameStarted) {
                            ...
                            JSONValue update_clients = ["action": "update_players_numbers"];
                            update_clients.object["totalPlayers"] = JSONValue(to!string(connectedClients.length + 1));
                            update_clients.object["readyPlayers"] = JSONValue(to!string(settings.readyPlayers));
                            logMessage(update_clients.toString(), fg.blue);
                            foreach(player_to_send; connectedClients) {
                                ...
                                player_to_send.send(update_clients.toString());
                            }
