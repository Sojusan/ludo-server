            ...
            if (pawn_clicked == pawn) {
                ...
                if (checkPawnOnPlayBoard(field)) {
                    if (!pawnCanMove(field, player, settings.rollResult, settings)) {
                        /// Player can end turn when he has a pawn on game borad but he can't move it the specidied
                        /// amount of fields
                        settings.rollResult = 0;
                        settings.players[player].playerCanEndTurn = true;
                    }
...
Nullable!FieldPawnsOwner getEnemyPlayerOnStartPoint(string player, string destination_field, FieldPawnsOwner[] field_info) {
